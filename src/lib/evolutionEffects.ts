// Evolution special effects implementation based on Cracked Royale mechanics
  ...
  switch (unit.cardId) {
    ...
    case "dart-goblin":
      ...
      const poisonDamage = Math.floor(
        unit.damage * (poisonStacks >= 7 ? 0.3 : poisonStacks >= 4 ? 0.2 : 0.1),
      );
      if (poisonDamage > 0) {
        result.statusEffectsToApply = [
          {
...
export function applyEvolutionOnDamage(
  ...
  switch (unit.cardId) {
    ...
    case "wizard":
      ...
      if (
        state.shieldActive &&
        unit.health - damageAmount <= unit.maxHealth * 0.5
      ) {
        state.shieldActive = false;
    ...
    case "royal-recruits":
      ...
      if (state.shieldActive && damageAmount >= 50) {
        // Shield hit
...
export function getEvolutionRageBonus(
  ...
  if (
    unit.cardId === "barbarians" &&
    state.rageEndTime &&
    currentTime < state.rageEndTime
  ) {
