...
import {
  ...
  SyncedGameState,
  GameStateDelta,
} from "@/hooks/useMultiplayerBattle";
...
export function GameUI({
  ...
  const [arenaHoverPos, setArenaHoverPos] = useState<{
    x: number;
    y: number;
  } | null>(null);
    ...
    if (Math.random() > 0.5) {
      setTimeout(
        () => {
          ...
          const randomEmote =
            aiEmotes[Math.floor(Math.random() * aiEmotes.length)];
  ...
  useEffect(() => {
      ...
      if (now - lastDeltaSyncRef.current >= DELTA_SYNC_INTERVAL) {
        ...
        if (gameState.gameStatus === "playing") {
          ...
          const delta = computeDelta(
            gameState.playerTowers.map((t) => ({
              id: t.id,
              health: t.health,
              maxHealth: t.maxHealth,
            })),
            gameState.enemyTowers.map((t) => ({
              id: t.id,
              health: t.health,
              maxHealth: t.maxHealth,
            })),
    ...
    if (gameState.gameStatus !== "playing" && onSyncGameState) {
      onSyncGameState({
        timestamp: Date.now(),
        playerTowers: gameState.playerTowers.map((t) => ({
          id: t.id,
          health: t.health,
          maxHealth: t.maxHealth,
        })),
        enemyTowers: gameState.enemyTowers.map((t) => ({
          id: t.id,
          health: t.health,
          maxHealth: t.maxHealth,
        })),
  ...
  const playerCrowns =
    3 - gameState.enemyTowers.filter((t) => t.health > 0).length;
  const enemyCrowns =
    3 - gameState.playerTowers.filter((t) => t.health > 0).length;

  // Handle game end with new BattleResults screen
  if (gameState.gameStatus !== "playing") {
    // Use real opponent data in multiplayer, otherwise generate random
    const enemyName =
      isMultiplayer && battleState
  ...
  const hasBonusZones = gameState.playerPlacementZones.some(
    (z) => z.reason === "tower-destroyed",
  );
      ...
      <div className="flex-1 flex flex-col items-center min-h-0">
        ...
        <div className="flex-1 flex items-start justify-center min-h-0 overflow-hidden">
          <Arena
            ...
            hoverPos={arenaHoverPos}
            onArenaMouseMove={setArenaHoverPos}
            onArenaMouseLeave={() => setArenaHoverPos(null)}
          />
        </div>
