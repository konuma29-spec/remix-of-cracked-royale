...
function getCardLevelsFromCopies(
...
function getTowerLevelsFromCopies(towerCopies: Record<string, number>): {
  princess: number;
  king: number;
} {
  return {
...
export function GameScreen() {
  ...
  const [megaDraftDeck, setMegaDraftDeck] = useState<string[] | null>(null);
  ...
  const handleGenerateReward = useCallback(
    (
      stars: number,
  ...
  const handleStartFriendlyBattle = useCallback(async () => {
    ...
    const opponentId = isChallenger
      ? acceptedBattle.to_user_id
      : acceptedBattle.from_user_id;
    const opponentName = isChallenger
      ? acceptedBattle.to_player_name
      : acceptedBattle.from_player_name;
    ...
    <>
      ...
      {screen === "home" && (
        <HomeNavigator
          ...
          onBattleWithDeck={(deck) => {
            setMegaDraftDeck(deck);
            setIsBossBattle(false);
            setScreen("loading");
          }}
          onBattleBossMode={() => {
      ...
      {screen === "battle" && (
        <GameUI
          playerDeck={megaDraftDeck ?? progress.currentDeck}
          cardLevels={
            isBossBattle
              ? Object.fromEntries(progress.currentDeck.map((id) => [id, 16]))
              : megaDraftDeck
                ? Object.fromEntries(megaDraftDeck.map((id) => [id, 11]))
                : getCardLevelsFromCopies(progress.cardCopies)
          }
          towerLevels={
            isBossBattle
              ? { princess: 16, king: 16 }
              : megaDraftDeck
                ? { princess: 11, king: 11 }
                : getTowerLevelsFromCopies(progress.towerCopies)
          }
          isMegaDraft={!!megaDraftDeck}
          ...
          opponentLevel={
            battleState?.opponentLevel || friendlyBattleData?.opponentLevel || 1
          }
        />
      )}
