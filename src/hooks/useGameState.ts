...
function createInitialTowers(
  ...
  const playerPrincessHealth = Math.floor(
    basePrincessHealth * princessMultiplier,
  );
  const playerPrincessDamage = Math.floor(
    basePrincessDamage * princessMultiplier,
  );
  ...
  const enemyLevel = isMultiplayer
    ? opponentLevel
  ...
  const enemyTowers: Tower[] = isBossBattle
    ? []
    : [
        {
          id: "enemy-king",
...
function createInitialPlacementZones(): {
  playerZones: PlacementZone[];
  enemyZones: PlacementZone[];
} {
...
function isOnBridge(x: number): boolean {
  return (
    (x >= LEFT_BRIDGE.minX && x <= LEFT_BRIDGE.maxX) ||
...
function wouldCrossRiver(fromY: number, toY: number): boolean {
  ...
  return (
    (fromY < RIVER_Y && toY >= RIVER_Y) || (fromY > RIVER_Y && toY <= RIVER_Y)
  );
}
...
function calculateMovement(
  ...
  if (!needsToCrossRiver || isOnBridge(currentX)) {
    ...
    if (len < 0.1)
      return { newX: currentX, newY: currentY, direction: unit.direction };
    ...
    if (wouldCrossRiver(currentY, newY) && !isOnBridge(newX)) {
      newY = unitOnPlayerSide
        ? RIVER_Y + RIVER_HALF_WIDTH + 1
        : RIVER_Y - RIVER_HALF_WIDTH - 1;
    }
  ...
  const bridgeY = unitOnPlayerSide
    ? RIVER_Y + RIVER_HALF_WIDTH
    : RIVER_Y - RIVER_HALF_WIDTH;
...
function createInitialState(
  ...
  const { hand: playerHand, remainingDeck: playerRemainingDeck } =
    drawHand(playerDeck);
  const { hand: enemyHand, remainingDeck: enemyRemainingDeck } =
    drawHand(enemyDeck);
...
export function useGameState(
  ...
  // Boss Battle ref
  const isBossBattleRef = useRef(isBossBattle);
  ...
  useEffect(() => {
    ...
    isMultiplayer,
    isHost,
    selectedTowerTroopId,
  ]);

  const addDamageNumber = useCallback(
    (position: Position, damage: number, isCritical = false) => {
  ...
  const trackCardDamage = useCallback(
    (cardId: string, damage: number, owner: "player" | "enemy") => {
  ...
  const getCardWithBalance = useCallback(
  ...
  const spawnUnit = useCallback(
    (
      ...
      const effectiveMoveSpeed = Math.round(
        balancedCard.moveSpeed * sizeSpeedMultiplier,
      );
  ...
  const addSpawnEffect = useCallback(
    (position: Position, owner: "player" | "enemy", emoji: string) => {
  ...
  const spawnBuilding = useCallback(
    (
      card: CardDefinition,
      position: Position,
      owner: "player" | "enemy",
  ...
  const playCard = useCallback(
    (cardIndex: number, position: Position) => {
      setGameState((prev) => {
        ...
        if (!isSpell && !isPositionInZones(position, prev.playerPlacementZones))
          return prev;
        ...
        const hasUnlockedEvolution = unlockedEvolutionsRef.current.includes(
          card.id,
        );
        ...
        const isEvolvedThisPlay =
          hasUnlockedEvolution &&
          cardHasEvolution &&
          currentPlayCount >= cyclesRequired;
  ...
  const playEnemyCard = useCallback(
          ...
          for (let i = 0; i < unitCount; i++) {
            const offset =
              unitCount > 1
                ...
                : { x: 0, y: 0 };
            const unitPos = {
              x: mirroredPosition.x + offset.x,
              y: mirroredPosition.y + offset.y,
            };
  ...
  const pendingReconciliationRef = useRef<{
    ...
    unitUpdates: Map<
      string,
      { position: Position; health: number; maxHealth: number }
    >;
  ...
  const applyHostState = useCallback(
    (hostState: {
      ...
      for (const tower of hostState.enemyTowers) {
        ...
        pending.towerHealth.set(ourTowerId, {
          health: tower.health,
  ...
  const applyHostDelta = useCallback(
    (delta: {
    ...
    const tick = (currentTime: number) => {
      ...
      setGameState((prev) => {
        ...
        const state: GameState = {
          ...
          playerUnits: prev.playerUnits.map((u) => ({
            ...u,
          ...
          activeSpells: prev.activeSpells.map((s) => ({
            ...s,
            effects: [...s.effects],
          })),
          playerPlacementZones: [...prev.playerPlacementZones],
          enemyPlacementZones: [...prev.enemyPlacementZones],
          playerCardCooldowns: prev.playerCardCooldowns.map((cd) =>
            Math.max(0, cd - delta),
          ),
          enemyCardCooldowns: prev.enemyCardCooldowns.map((cd) =>
            Math.max(0, cd - delta),
          ),
        };
        ...
        if (isMultiplayerRef.current && !isHostRef.current) {
          ...
          for (const removedId of pending.unitRemovals) {
            state.playerUnits = state.playerUnits.filter(
              (u) => u.id !== removedId,
            );
            state.enemyUnits = state.enemyUnits.filter(
              (u) => u.id !== removedId,
            );
          }
        ...
        state.playerElixir = Math.min(
          10,
          state.playerElixir + elixirRate * delta,
        );
        state.enemyElixir = Math.min(
          10,
          state.enemyElixir + elixirRate * delta,
        );
        ...
        const updatePlacementZonesForPlayer = () => {
          ...
          const enemyLeftPrincess = state.enemyTowers.find(
            (t) => t.id === "enemy-princess-left",
          );
          const enemyRightPrincess = state.enemyTowers.find(
            (t) => t.id === "enemy-princess-right",
          );

          const leftDestroyed =
            enemyLeftPrincess && enemyLeftPrincess.health <= 0;
          const rightDestroyed =
            enemyRightPrincess && enemyRightPrincess.health <= 0;
        ...
        const updatePlacementZonesForEnemy = () => {
          ...
          const playerLeftPrincess = state.playerTowers.find(
            (t) => t.id === "player-princess-left",
          );
          const playerRightPrincess = state.playerTowers.find(
            (t) => t.id === "player-princess-right",
          );

          const leftDestroyed =
            playerLeftPrincess && playerLeftPrincess.health <= 0;
          const rightDestroyed =
            playerRightPrincess && playerRightPrincess.health <= 0;
        ...
        // Only run AI in single-player mode and not in boss battle
        if (!isMultiplayerRef.current && !isBossBattleRef.current) {
          const aiDecision = makeAIDecision(state, aiLastPlayTime.current);
          if (
            aiDecision.shouldPlay &&
            aiDecision.card &&
            aiDecision.position !== undefined &&
            aiDecision.cardIndex !== undefined
          ) {
            // Check AI cooldown
            if (state.enemyCardCooldowns[aiDecision.cardIndex] <= 0) {
              ...
              if (
                ...
                for (let i = 0; i < unitCount; i++) {
                  const offset =
                    unitCount > 1
                      ...
                      : { x: 0, y: 0 };
                  const unitPos = {
                    x: aiDecision.position.x + offset.x,
                    y: aiDecision.position.y + offset.y,
                  };
                  const unit = spawnUnit(
                    aiDecision.card,
                    unitPos,
                  ...
                  unit.health = aiDecision.card.health;
                  unit.maxHealth = aiDecision.card.health;
                  newUnits.push(unit);
                }
                addSpawnEffect(
                  aiDecision.position,
                  "enemy",
                  aiDecision.card.emoji,
                );
                state.enemyUnits = [...state.enemyUnits, ...newUnits];
                ...
                if (nextCard) {
                  ...
                  state.enemyDeck = [
                    ...state.enemyDeck.slice(1),
                    aiDecision.card,
                  ];
                  state.enemyCardCooldowns[aiDecision.cardIndex] =
                    nextCard.deployCooldown;
                }
              }
            }
          }
        ...
        // Boss Battle: spawn a new boss whenever the arena is clear of enemies
        if (isBossBattleRef.current) {
          const aliveEnemies = state.enemyUnits.filter((u) => u.health > 0);
          if (aliveEnemies.length === 0) {
            const bossId =
              BOSS_TROOP_IDS[Math.floor(Math.random() * BOSS_TROOP_IDS.length)];
            const bossCard = getCardById(bossId);
            if (bossCard) {
              const spawnPos = {
                x: ARENA_WIDTH / 2 + (Math.random() - 0.5) * 60,
                y: 80,
              };
              const bossUnit = spawnUnit(
                bossCard,
                spawnPos,
                "enemy",
                BOSS_LEVEL,
              );
              addSpawnEffect(spawnPos, "enemy", "💀");
              state.enemyUnits = [...state.enemyUnits, bossUnit];
            }
          }
        }
        ...
        state.playerUnits = state.playerUnits.map((unit) => {
          ...
          const isFrozen = unit.statusEffects.some(
            (e) => e.type === "freeze" || e.type === "stun",
          );
          if (isFrozen) {
            unit.state = "idle";
            return unit;
          ...
          const validEnemyUnits = state.enemyUnits.filter((u) => {
            ...
            if (u.abilityState?.type === "cloak" && u.abilityState.isActive)
              return false;
            return true; // 'both' targets everything
          });

          const validEnemyTowers =
            unit.targetType === "air"
          ...
          if (unit.targetType === "buildings") {
            ...
            if (unit.targetId) {
              currentTarget =
                allBuildingTargets.find((t) => t.id === unit.targetId) || null;
              if (currentTarget) {
            ...
            if (currentTarget) {
              ...
              if (distToTarget <= unit.range) {
                ...
                const rageBonus = getEvolutionRageBonus(
                  unit,
                  evolutionStateRef.current,
                  now,
                );
                const effectiveAttackSpeed =
                  unit.attackSpeed * rageBonus.attackSpeedMultiplier;
                if (now - unit.lastAttackTime > 1000 / effectiveAttackSpeed) {
                  ...
                  if (unit.isEvolved) {
                    ...
                    if (evoEffect.damageMultiplier)
                      damage = Math.round(damage * evoEffect.damageMultiplier);
                  }
          ...
          if (unit.targetId) {
            const allTargets: (Unit | Tower | Building)[] = [
              ...validEnemyUnits,
              ...validEnemyBuildings,
              ...validEnemyTowers,
            ];
              ...
              if (now - unit.lastAttackTime > 1000 / effectiveAttackSpeed) {
                ...
                if (unit.isEvolved) {
                  const evoEffect = applyEvolutionOnAttack(
                    unit,
                    {
                      ...
                      position: currentTarget.position,
                      isFlying:
                        "isFlying" in currentTarget
                          ? (currentTarget as Unit).isFlying
                          : false,
                    },
                    evolutionStateRef.current,
                    now,
                  ...
                  if (evoEffect.damageMultiplier)
                    damage = Math.round(damage * evoEffect.damageMultiplier);
                  if (evoEffect.healthHeal)
                    unit.health = Math.min(
                      unit.maxHealth * 2,
                      unit.health + evoEffect.healthHeal,
                    );
                  if (
                    evoEffect.statusEffectsToApply &&
                    "statusEffects" in currentTarget
                  ) {
                    evoEffect.statusEffectsToApply.forEach((sa) => {
                      if (sa.targetId === currentTarget!.id)
                        (currentTarget as Unit).statusEffects.push(sa.effect);
                    });
                  }
                      ...
                      if (spawnCard) {
                        for (let i = 0; i < spawn.count; i++) {
                          ...
                          if (spawn.owner === "player")
                            state.playerUnits.push(spawnedUnit);
                ...
                if (unit.splashRadius && unit.splashRadius > 0) {
                  ...
                  splashTargets.forEach((target) => {
                    const distToSplash = getDistance(
                      currentTarget!.position,
                      target.position,
                    );
                  ...
                  if (
                    "isEvolved" in currentTarget &&
                    (currentTarget as Unit).isEvolved
                  ) {
                    const reduction = getEvolutionDamageReduction(
                      currentTarget as Unit,
                      evolutionStateRef.current,
                    );
                    if (reduction > 0)
                    ...
                    evoEffect.unitsToSpawn.forEach((spawn) => {
                      ...
                      if (spawnCard) {
                        for (let i = 0; i < spawn.count; i++) {
                          ...
                          if (spawn.owner === "enemy")
                            state.enemyUnits.push(spawnedUnit);
        ...
        state.activeSpells = state.activeSpells
          .map((spell) => {
            ...
            const allEnemyUnits =
              spell.owner === "player" ? state.enemyUnits : state.playerUnits;
            const allEnemyTowers =
              spell.owner === "player" ? state.enemyTowers : state.playerTowers;
            const allEnemyBuildings =
              spell.owner === "player"
            ...
            const targetsInRange = [
              ...
              ...allEnemyTowers.filter(
                (t) =>
                  t.health > 0 &&
                  getDistance(spell.position, t.position) <= spell.radius,
              ),
              ...allEnemyBuildings.filter(
                (b) =>
                  b.health > 0 &&
                  getDistance(spell.position, b.position) <= spell.radius,
              ),
            ];
                ...
                if (effect.type === "freeze" || effect.type === "stun") {
                  ...
                  allEnemyUnits
                    .filter(
                      (u) =>
                        u.health > 0 &&
                        getDistance(spell.position, u.position) <= spell.radius,
                    )
                    .forEach((unit) => {
                      const existingEffect = unit.statusEffects.find(
                        (e) =>
                          e.sourceId === spell.id && e.type === effect.type,
                      );
                      if (!existingEffect) {
                        unit.statusEffects.push({
                          ...
                          value: effect.value,
                          remainingDuration:
                            effect.duration || spell.remainingDuration,
                          sourceId: spell.id,
                        });
                      }
                    });
                  ...
                  allEnemyUnits
                    ...
                    .forEach((unit) => {
                      const existingEffect = unit.statusEffects.find(
                        (e) => e.sourceId === spell.id && e.type === "slow",
                      );
                      if (!existingEffect) {
                        unit.statusEffects.push({
                          type: "slow",
          ...
          .filter(
            (spell) =>
              spell.remainingDuration > 0 ||
              !spell.hasAppliedInstant ||
              (spell.remainingDuration === 0 && spell.hasAppliedInstant),
          );
              ...
              if (spawnCard) {
                ...
                for (let i = 0; i < spawnCount; i++) {
                  ...
                  const newUnit = spawnUnit(
                    spawnCard,
                    { x: unit.position.x + offsetX, y: spawnY },
                    owner,
                    unit.level,
                  );
          ...
          return buildings
            .map((building) => {
              ...
              if (
                ...
                if (
                  now - updated.lastSpawnTime >
                  (updated.spawnInterval || 5) * 1000
                ) {
              ...
              if (
                ...
                const enemyUnits =
                  owner === "player" ? state.enemyUnits : state.playerUnits;
                const enemyTowers =
                  owner === "player" ? state.enemyTowers : state.playerTowers;
                ...
                const validTowerTargets = isSiegeBuilding
                  ? enemyTowers.filter(
                      (t) =>
                        t.health > 0 &&
                        getDistance(updated.position, t.position) <=
                          updated.range,
                    )
                  : [];
                ...
                const validBuildingTargets = isSiegeBuilding
                  ? enemyBuildings.filter(
                      (b) =>
                        b.health > 0 &&
                        getDistance(updated.position, b.position) <=
                          updated.range,
                    )
                  : [];
                ...
                const validUnitTargets = enemyUnits.filter((u) => {
                  if (u.health <= 0) return false;
                  if (updated.targetType === "ground" && u.isFlying)
                    return false;
                  ...
                  if (
                    isSiegeBuilding &&
                    (validTowerTargets.length > 0 ||
                      validBuildingTargets.length > 0)
                  )
                    return false;
                  return (
                    getDistance(updated.position, u.position) <= updated.range
                  );
                });
                ...
                if (isSiegeBuilding) {
                  if (hasTowerTarget) {
                    const closestTower = validTowerTargets.reduce(
                      (closest, tower) =>
                        getDistance(updated.position, tower.position) <
                        getDistance(updated.position, closest.position)
                          ? tower
                          : closest,
                    );
                    updated.targetId = closestTower.id;
                  } else if (hasBuildingTarget) {
                    const closestBuilding = validBuildingTargets.reduce(
                      (closest, building) =>
                        getDistance(updated.position, building.position) <
                        getDistance(updated.position, closest.position)
                          ? building
                          : closest,
                    );
                ...
                if (
                  (hasUnitTarget || hasTowerTarget || hasBuildingTarget) &&
                  now - updated.lastAttackTime > 1000 / updated.attackSpeed
                ) {
                  updated.lastAttackTime = now;
                  const buildingDamage = Math.round(
                    updated.damage * DAMAGE_MULTIPLIER,
                  );
                  ...
                  if (hasTowerTarget) {
                    ...
                    if (target.health <= 0) {
                      ...
                      if (target.type === "princess") {
                        const kingTower = enemyTowers.find(
                          (t) => t.type === "king",
                        );
                    ...
                    const target = validBuildingTargets.reduce(
                      (closest, building) =>
                        getDistance(updated.position, building.position) <
                        getDistance(updated.position, closest.position)
                          ? building
                    ...
                    const target = validUnitTargets.reduce((closest, unit) =>
                      getDistance(updated.position, unit.position) <
                      getDistance(updated.position, closest.position)
                        ? unit
                        : closest,
                    );
                    if (updated.splashRadius && updated.splashRadius > 0) {
                      validUnitTargets.forEach((t) => {
                        if (
                          getDistance(target.position, t.position) <=
                          updated.splashRadius!
                        ) {
        ...
        state.playerBuildings = updateBuildings(
          state.playerBuildings,
          "player",
        );
        ...
        state.playerTowers.forEach((tower) => {
          ...
          if (now - tower.lastAttackTime > tower.attackCooldown) {
            ...
            if (tower.type === "king" && tower.isActivated) {
              ...
              const princessTowers = state.playerTowers.filter(
                (t) => t.type === "princess" && t.health > 0,
              );
              ...
              enemies = state.enemyUnits.filter((u) => {
                ...
                if (
                  getDistance(tower.position, u.position) <= tower.attackRange
                )
                  return true;
                ...
                for (const princess of princessTowers) {
                  if (
                    getDistance(princess.position, u.position) <=
                    princess.attackRange
                  ) {
                    return true;
                  }
                }
              ...
              enemies = state.enemyUnits.filter((u) => {
                if (u.health <= 0) return false;
                if (getDistance(tower.position, u.position) > tower.attackRange)
                  return false;

                // Princess towers cannot fire at units on/past the bridge (enemy's side)
                if (tower.type === "princess") {
                  const isOnOrPastBridge =
                    u.position.y <= RIVER_Y + RIVER_HALF_WIDTH;
            ...
            if (enemies.length > 0) {
              ...
              enemies.sort(
                (a, b) =>
                  getDistance(tower.position, a.position) -
                  getDistance(tower.position, b.position),
              );
        ...
        state.enemyTowers.forEach((tower) => {
          ...
          if (now - tower.lastAttackTime > tower.attackCooldown) {
            ...
            if (tower.type === "king" && tower.isActivated) {
              // ACTIVATED KING TOWER: Can attack enemies near either princess tower OR in king range
              const princessTowers = state.enemyTowers.filter(
                (t) => t.type === "princess" && t.health > 0,
              );
              ...
              if (proj.type === "pancake") {
                ...
                const friendlyUnits =
                  proj.owner === "player"
                    ? state.playerUnits
                    : state.enemyUnits;
                const target = friendlyUnits.find(
                  (u) => u.id === proj.targetId,
                );
                if (target && target.health > 0) {
                  ...
                  target.maxHealth = Math.floor(
                    target.maxHealth * buffMultiplier,
                  );
                  // Visual feedback
        ...
        const processChampionAbilities = (
          units: Unit[],
          owner: "player" | "enemy",
          enemyUnits: Unit[],
        ) => {
            ...
            switch (ability.type) {
              case "dash-chain": {
                ...
                if (unit.state === "attacking" && deadEnemies.length > 0) {
                  ...
                  const aliveEnemies = enemyUnits.filter(
                    (e) => e.health > 0 && e.id !== unit.targetId,
                  );
                  if (aliveEnemies.length > 0) {
                    ...
                    if (closestEnemy) {
                      ...
                      const dashDamage = Math.round(
                        unit.damage * 0.5 * DAMAGE_MULTIPLIER,
                      );
              ...
              case "cloak": {
                ...
                if (
                  !ability.isActive &&
                  healthPercent < 0.7 &&
                  now - ability.lastActivationTime > 10000
                ) {
              ...
              case "soul-summon": {
                ...
                // This matches Cracked Royale's behavior where the ability button activates the summon
                const SOUL_RANGE = 100;
                const soulsGained = deadEnemies.filter(
                  (e) => getDistance(unit.position, e.position) <= SOUL_RANGE,
                ).length;
              ...
              case "drill": {
                ...
                if (!ability.hasTriggered && healthPercent <= 0.3) {
                  ...
                  const BURROW_DAMAGE = Math.round(
                    unit.damage * 0.8 * DAMAGE_MULTIPLIER,
                  );
                  enemyUnits
                    .filter(
                      (e) =>
                        e.health > 0 &&
                        getDistance(unit.position, e.position) <= 60,
                    )
                    .forEach((enemy) => {
              ...
              case "guardian": {
                ...
                if (
                  !ability.hasTriggered &&
                  healthPercent <= 0.5 &&
                  now - ability.lastActivationTime > 12000
                ) {
                  ability.hasTriggered = true;
                  ability.lastActivationTime = now;
              ...
              case "reflect": {
                ...
                if (
                  !ability.isActive &&
                  now - ability.lastActivationTime > 8000
                ) {
                  ability.isActive = true;
        ...
        const newPlayerUnits = processChampionAbilities(
          state.playerUnits,
          "player",
          state.enemyUnits,
        );
        const newEnemyUnits = processChampionAbilities(
          state.enemyUnits,
          "enemy",
          state.playerUnits,
        );
        ...
        if (towerTroop.hasPancakeBuff && towerTroop.pancakeInterval) {
          ...
          if (now - lastPancakeThrowTime.current > pancakeInterval) {
            ...
            const alivePrincessTowers = state.playerTowers.filter(
              (t) => t.type === "princess" && t.health > 0,
            );
            ...
            if (alivePrincessTowers.length > 0 && buffableUnits.length > 0) {
              ...
              const target = buffableUnits.reduce((closest, unit) => {
                ...
                const distToClosest = getDistance(
                  tower.position,
                  closest.position,
                );
        ...
        const processDeathEffects = (
          units: Unit[],
          owner: "player" | "enemy",
        ) => {
          const allFriendlyUnits =
            owner === "player" ? state.playerUnits : state.enemyUnits;

          units.forEach((unit) => {
            // Witch evo: heal parent Witch when her spawned skeletons die
            if (unit.health <= 0 && unit.parentId) {
              const parent = allFriendlyUnits.find(
                (u) => u.id === unit.parentId && u.health > 0,
              );
              if (parent && parent.isEvolved && parent.cardId === "witch") {
                ...
                parent.health = Math.min(
                  parent.maxHealth * 1.24,
                  parent.health + healAmount,
                ); // Can overheal to 124%
            ...
            if (unit.health <= 0 && unit.isEvolved) {
              const deathEffect = applyEvolutionOnDeath(
                unit,
                evolutionStateRef.current,
                now,
              );
        ...
        const checkTowerDestruction = (
          towers: Tower[],
          side: "player" | "enemy",
        ) => {
          towers.forEach((tower) => {
            const prevHealth =
              prevTowerHealthRef.current.get(tower.id) ?? tower.maxHealth;
        ...
        // Boss Battle win conditions: survive 3 minutes or die
        if (isBossBattleRef.current) {
          if (playerKing && playerKing.health <= 0) {
            ...
            state.gameStatus = "player-wins"; // Survived!
          }
        } else {
          ...
          if (enemyKing && enemyKing.health <= 0) {
            ...
            const playerTowersAlive = state.playerTowers.filter(
              (t) => t.health > 0,
            ).length;
            const enemyTowersAlive = state.enemyTowers.filter(
              (t) => t.health > 0,
            ).length;
            ...
            if (playerTowersDestroyed > enemyTowersDestroyed) {
              ...
              const allTowers = [
                ...state.playerTowers
                  .filter((t) => t.health > 0)
                  .map((t) => ({ ...t, side: "player" as const })),
                ...state.enemyTowers
                  .filter((t) => t.health > 0)
                  .map((t) => ({ ...t, side: "enemy" as const })),
              ];

              let lowestHealthTower: {
                id: string;
              ...
              for (const tower of allTowers) {
                ...
                if (
                  !lowestHealthTower ||
                  healthPercent < lowestHealthTower.healthPercent
                ) {
                  lowestHealthTower = {
                    id: tower.id,
                    healthPercent,
              ...
              if (lowestHealthTower) {
                ...
                if (lowestHealthTower.side === "player") {
                  const tower = state.playerTowers.find(
                    (t) => t.id === lowestHealthTower!.id,
                  );
                  if (tower) tower.health = 0;
                  state.gameStatus = "enemy-wins";
                } else {
                  const tower = state.enemyTowers.find(
                    (t) => t.id === lowestHealthTower!.id,
                  );
                  if (tower) tower.health = 0;
                  state.gameStatus = "player-wins";
  ...
  const activateChampionAbility = useCallback(
    (unitId: string) => {
      setGameState((prev) => {
        ...
        if (
          cooldownMs > 0 &&
          now - abilityState.lastActivationTime < cooldownMs
        ) {
          return prev;
        }
        ...
        switch (abilityState.type) {
          case "dash-chain": {
            ...
            if (!abilityState.isActive) {
              ...
              const enemies = state.enemyUnits.filter(
                (e) => e.health > 0 && !e.isFlying,
              );
              if (enemies.length > 0) {
                ...
                if (getDistance(unit.position, closest.position) <= dashRange) {
                  ...
                  const dashDamage = Math.round(
                    unit.damage * 2 * DAMAGE_MULTIPLIER,
                  );
            ...
            if (abilityState.stacks >= minSouls) {
              ...
              if (skeletonCard) {
                ...
                for (let i = 0; i < numToSpawn; i++) {
                  ...
                  state.playerUnits = [
                    ...state.playerUnits,
                    spawnUnit(skeletonCard, spawnPos, "player", unit.level),
                  ];
                }
                addSpawnEffect(unit.position, "player", "💀");
            ...
            if (!abilityState.isActive) {
              ...
              state.enemyUnits
                .filter(
                  (e) =>
                    e.health > 0 &&
                    getDistance(unit.position, e.position) <= 80,
                )
                .forEach((enemy) => {
  ...
  function getChampionAbilityInfo(abilityType: string): {
    cooldown: number;
    elixirCost: number;
  } {
    switch (abilityType) {
      case "dash-chain":
        return { cooldown: 8, elixirCost: 1 };
      case "cloak":
        return { cooldown: 17, elixirCost: 1 };
      case "soul-summon":
        return { cooldown: 0, elixirCost: 2 };
      case "drill":
        return { cooldown: 15, elixirCost: 1 };
      case "guardian":
        return { cooldown: 0, elixirCost: 3 };
      case "reflect":
        return { cooldown: 17, elixirCost: 1 };
      default:
        return { cooldown: 0, elixirCost: 0 };
    }
  }
